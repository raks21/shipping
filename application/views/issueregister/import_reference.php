    ...
    <section class="wrapper">
        ...
        <form class="form-horizontal style-form" method="POST" enctype="multipart/form-data">
            <div class="row" style="background:ccc;"><h3 style="float:left; padding-left: 10px;"><?php echo $page_title; ?> </h3>
                <button type="submit" name="import_consignment" onclick="$('#loading123').show();" class="btn btn-primary pull-right" style=" margin: 11px 6px;">+ Import</button>
            </div>
            <div class="row mb">
                <!-- page start-->
                <div class="col-lg-12">
                                ...
                                <div class="col-sm-12">
                                    <select class="form-control" name="prefix">
                                        ...
                                        foreach ($get_prefix as $value) {
                                            ?>
                                            <option <?php if ($value->Prefix_Id == $this->session->userdata('impref_prefix')) { ?>selected="selected"<?php } ?>  value='<?php echo $value->Prefix_Id ?>'><?php echo $value->Prefix_Name ?></option>
                                            <?php
                                        }
                                        ?>
                                    </select>
                                ...
                                <div class="col-sm-12">
                                    <select class="form-control" name="partyname" id="partyname">
                                        ...
                                        foreach ($get_party as $value) {
                                            ?>
                                            <option <?php if ($value->Party_id == $this->session->userdata('impref_partyname')) { ?>selected="selected"<?php } ?>  value='<?php echo $value->Party_id ?>'><?php echo $value->Party_Name ?></option>
                                            <?php
                                        }
                                        ?>
                                    </select>
                ...
                <div class="col-lg-12">
                    <div>
                        <h4><i class="fa fa-angle-right"></i>  Import Excel Format with Reference No :</h4>
                        <div class="alert alert-warning alert-dismissable">
                            Consignee_Name | Consignee_Address1 | Consignee_Address2 | Consignee_Address3 | Consignee_Place | Consignee_Pincode | Consignee_Weight | No_Of_Pieces | Consignment_No | Reference_No
                        </div>
                    </div>
                </div>
                <!-- col-lg-12-->
